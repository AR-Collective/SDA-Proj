...
def handle_missing_values_in_column(
    ...
    if strategy not in FILLING_STRATEGIES:
        raise ValueError(f"Unknown strategy: {strategy}. Available: {list(FILLING_STRATEGIES.keys())}")
...
def handle_missing_values_in_dataframe(
    ...
    processed_cols = dict(map(
        lambda col: (col, handle_missing_values_in_column(result[col], numeric_strategy)),
        numeric_cols
    ))
...
def get_cleaning_summary(df_before: pd.DataFrame, df_after: pd.DataFrame) -> Dict:
    ...
    improved_cols = list(filter(
        lambda col: col in missing_before and col in missing_after and missing_after[col] < missing_before[col],
        df_before.columns
    ))
